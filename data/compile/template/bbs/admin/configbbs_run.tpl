...
<body>
<div class="wrap">
	<form class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=configbbs&a=dorun'; ?>" method="post">
  ...
  if ($group['type'] == $type) { ?>
									<label><input class="J_check" data-yid="J_check_timging_<?php echo htmlspecialchars($type, ENT_QUOTES, 'UTF-8');?>" type="checkbox" name="timing_groups[]" value="<?php echo htmlspecialchars($group['gid'], ENT_QUOTES, 'UTF-8');?>"<?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray($group['gid'], $config['post.timing.groups'])), ENT_QUOTES, 'UTF-8');?> /><span><?php echo htmlspecialchars($group['name'], ENT_QUOTES, 'UTF-8');?></span></label>
									<?php  } 
  } ?>
								</dd>
							</dl>
					...
					<td>
						<div class="user_group J_check_wrap">
							...
							<dl>
								...
								<?php  foreach ($groups as $group) { 
  if ($group['type'] == $type) { ?>
									<label><input class="J_check" data-yid="J_check_<?php echo htmlspecialchars($type, ENT_QUOTES, 'UTF-8');?>" type="checkbox" name="check_groups[]" value="<?php echo htmlspecialchars($group['gid'], ENT_QUOTES, 'UTF-8');?>"<?php echo htmlspecialchars(Pw::ifcheck(Pw::inArray($group['gid'], $config['post.check.groups'])), ENT_QUOTES, 'UTF-8');?> /><span><?php echo htmlspecialchars($group['name'], ENT_QUOTES, 'UTF-8');?></span></label>
									<?php  } 
  } ?>
								</dd>
							</dl>
