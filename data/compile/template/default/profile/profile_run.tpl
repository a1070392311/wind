...
?>
		<li class="<?php echo htmlspecialchars($_item['current'], ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo htmlspecialchars(WindUrlHelper::createUrl($_item['url'], array('_tab' => $key)), ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars($_item['title'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php } else { ?>
...
<?php 
	...
	<form class="J_profile_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&a=dorun'; ?>" method="post">
	<div class="profile_ct">
		...
		<dl class="cc">
			...
			<dd class="J_region_set">
				...
				<a data-pid="<?php echo htmlspecialchars($location[0]['areaid'], ENT_QUOTES, 'UTF-8');?>" data-cid="<?php echo htmlspecialchars($location[1]['areaid'], ENT_QUOTES, 'UTF-8');?>" data-did="<?php echo htmlspecialchars($location[2]['areaid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=area'; ?>" class="J_region_change">更改</a>
		...
		<dl class="cc">
			...
			<dd class="J_region_set">
				...
				<a data-pid="<?php echo htmlspecialchars($hometown[0]['areaid'], ENT_QUOTES, 'UTF-8');?>" data-cid="<?php echo htmlspecialchars($hometown[1]['areaid'], ENT_QUOTES, 'UTF-8');?>" data-did="<?php echo htmlspecialchars($hometown[2]['areaid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=area'; ?>" class="J_region_change">更改</a>
...
<div style="display:none;position:absolute;width:600px;" class="core_pop_wrap" id="J_ubbdemo_pop">
	...
	<div class="pop_cont" style="height: 250px;overflow-x: hidden;overflow-y: auto;position: relative;">
		<table class="pop_design_tablelist">
			...
			<tr>
				<td>图片</td>
				<th>[img]<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/'.Wekit::C('site', 'theme.site.default').'/images'; ?>/logo.png[/img]</th>
				<td><img src="<?php echo Wind::getComponent('response')->getData('G', 'url', 'themes').'/site/'.Wekit::C('site', 'theme.site.default').'/images'; ?>/logo.png"></td>
			</tr>
			<tr>
				<td>字号</td>
