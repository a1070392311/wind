...
<div class="nav">
	<ul class="cc">
		<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=config'; ?>">站点信息</a></li>
		<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=config&a=site'; ?>">全局参数</a></li>
	</ul>
</div>
<form method="post" class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=config&c=config&a=dorun'; ?>">
	...
	<tr>
		...
		<td>
			<ul id="J_status_type" class="single_list cc">
				<li><label><input data-title="s1" data-type="" name="visitState" type="radio" value="0"<?php echo htmlspecialchars(Pw::ifcheck(!$config['visit.state']), ENT_QUOTES, 'UTF-8');?>><span>完全开放</span></label></li>
				<li><label><input data-title="s2" data-type="J_status_s1,J_status_s2" name="visitState" type="radio" value="1"<?php echo htmlspecialchars(Pw::ifcheck($config['visit.state']==1), ENT_QUOTES, 'UTF-8');?>><span>内部开放</span></label></li>
				<li><label><input data-title="s3" data-type="J_status_s2" name="visitState" type="radio" value="2"<?php echo htmlspecialchars(Pw::ifcheck($config['visit.state']==2), ENT_QUOTES, 'UTF-8');?>><span>完全关闭</span></label></li>
			</ul>
		</td>
...
<table width="100%" id="J_status_s1" class="J_status_tbody">
	...
	<col />
		...
		<?php foreach($groupTypes as $type => $typeName){ 
			...
			$checked = Pw::inArray($type,$config['visit.group']);?>
					<dl>
		...
		if($group['type'] == $type){
			...
			$checked = Pw::inArray($group['gid'],$config['visit.gid']);?>
